...
def run(start, end, index=0, isHam=False):
    ...
    for ff in range(start,end):
           ...
           mylock.acquire()
           result_file = open('./result_threshold_ham.txt', 'a')
           result_file.write(result)
           result_file.close()
           mylock.release()

t0 = t(target=run,args=(1,6,0,True))
t1 = t(target=run,args=(6,11,1,True))
t2 = t(target=run,args=(11,16,2,True))
t3 = t(target=run,args=(16,21,3,True))
t0.start()
t1.start()
t2.start()
t3.start()
